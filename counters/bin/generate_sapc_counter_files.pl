...
my $data_dir     = "/eniq/data/pmdata/$oss_id/sapc/dir1";
my $counters_dir = '/eniq/home/dcuser/counters/nodes/sapc';
...
my $fileFormatVersion = '32.401 V5.0';
my %node_version_for  = (
   sapc_01 => '16B',
   sapc_02 => '16B',
);
...
for my $day_index ( $day_start .. $day_end ) {
   ...
   for my $start_time ( sort keys %end_time_for ) {
      ...
      for my $sapc_id (@sapc_range) {
         ...
         my $dnPrefix  = "DC=ims.ericsson.se,g3SubNetwork=EPC,g3ManagedElement=$sapc_id";
         ...
         for my $mo_type ( sort keys %formulas_for ) {
            my $sapc_file = "$data_dir/A$date.$start_time-${end_time}_${sapc_id}_$mo_type";
            open my $SAPC_FH, '>', "$sapc_file" or croak "Cannot open file $sapc_file, $!";
            print {$SAPC_FH} format_header( $dnPrefix, $startdate, $fileFormatVersion, $vendorName);
            ...
            for my $counter (@counters_list) {
               ...
               print_measurement_info_start( $SAPC_FH, $rop_length_in_seconds, $stopdate, ($counter) );
               for my $processor (@processors) {
                  my %formula_values = get_values_for($formulas_for{$mo_type});
                  print_measurement_info( $SAPC_FH, "Pm${mo_type}=DEFAULT, Source=$processor", $formula_values{$counter});
               }

               print_measurement_info_end( $SAPC_FH );
            }

            print {$SAPC_FH} ENIQ::Reports::format_footer($stopdate), "\n";

...
for my $node_config_file (@node_config_files) {
   ...
   my $fdn        = "SubNetwork=$config->{ROOT_MO},SubNetwork=$rnc_id,MeContext=$node";
   ...
   my $node_file = "$output_dir/A${file_date_time}_$config->{dnPrefix}_statsfile.xml";
   mkpath($output_dir);

   open $config->{FILE_HANDLE}, '>', "$node_file" or croak "Cannot open file $node_file, $!";      
   ...
   close $config->{FILE_HANDLE} or croak "Cannot close file $node_file, $!";   
}

...
my $usage = <<"USAGE";
 This script creates and populates the NR RAN topology files with sample data.
...
sub nr_topology {
   ...
   return <<"NR_TOPOLOGY";
  <mo fdn="$args{fdn}" mimName="NR_NODE_MODEL">
  ...
  <mo fdn="$args{fdn},ManagedElement=1,GNodeBFunction=1" mimName="NR_NODE_MODEL">
    <attr name="gNBId">$args{node}</attr>
    <attr name="gNodeBPlmnId">
      <struct>

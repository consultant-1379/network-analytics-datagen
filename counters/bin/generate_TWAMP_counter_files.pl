...
for my $node_config_file (@node_config_files) {
   ...
   my $output_dir = "/eniq/data/pmdata/$config->{METADATA}{OSS_ID}/$config->{METADATA}{COMMON_OUTPUT_DIR}/dir1/";

   print "config :\n", Dump( $config ) if $debug;   
 
   my $new_config;
 
   (my $file_date_time, $new_config->{cbt}, $new_config->{ts}, $new_config->{beginTime}, $new_config->{endTime} ) = get_time_info($config->{METADATA}{ROP_LENGTH}, $config->{ATTRIBUTES}{timeZone}, 'UTC'); # TCU TechPack uses UTC 
   ...
   for my $mo_type (sort keys %moids_for) {
      next unless $mo_type eq 'TwampTestSession';               # TWAMP handled here separately due to TCU TechPack using UTC and RadioNode using local time
      ...
      for my $moid (@node_mos) {
         debug("         moid:  $moid");
         set_twamp_values($moid) if $mo_type eq 'TwampTestSession';         
         $new_config->{measObjLdn} = $moid;

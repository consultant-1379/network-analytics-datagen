...
my $usage = <<"USAGE";
 This script creates and populates the HSS topology files with sample data.
...
for my $node_config_file (@node_config_files) {
   ...
   print {$TOPOLOGY_FH} topology( 
                                 ...
                                 $config->{ATTRIBUTES}{neMIMversion},
                                 $config->{ATTRIBUTES}{sourceType},
                                );
...
sub topology {
   my ($fdn, $user_label, $site, $node_version, $managed_element_type, $neMIMversion, $sourceType ) = @_;
   my $me_type;
...
ME_TYPE
   ...
   return <<"NE_TOPOLOGY";
   <mo fdn="$fdn" mimName="ONRM" mimVersion="$neMIMversion">

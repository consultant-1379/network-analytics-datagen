#!/usr/bin/perl
#
# This script creates a model of the managed object hierarchy for MGWs in a Core network.
#
...
#
# An MGW is represented by
#    ManagedObjects/SubNetwork=ONRM_RootMo/ManagedElement=mgw_01
#
...
clean_old_paths(root_dir => $root_dir, node_prefix => 'MeContext\=mgw_') if -d $root_dir and $clean;
...
my %node_model = (
   ...
   CONFIGURATION => {
      ATTRIBUTES                => {
         ...
         managedElementType                 => 'MGW,Bgf,Mrfp',
         neMIMversion                       => '14.2.5',
         nodeVersion                        => '17A',
         siteRef                            => '200000',

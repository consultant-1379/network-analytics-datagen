...
for my $node_config_file (@node_config_files) {
   ...
   print {$TOPOLOGY_FH} "SubNetwork=$config->{ROOT_MO},SubNetwork=$node,ManagedElement=$node|$node|$config->{nodeVersion}|$config->{managedElementType}|$config->{sourceType}|$config->{TRX}|SubNetwork=$config->{ROOT_MO},Site=$config->{siteRef}\n";
   close $TOPOLOGY_FH or croak "Cannot close file $topology_file, $!";

...
my $node_counter_dir     = "/eniq/home/dcuser/counters/nodes/bts/Counters";
my @node_config_files    = get_configuration_info( root_dir => $root_dir, node_type => 'bts' );
...
for my $node_config_file (@node_config_files) {
   ...
   next unless $config->{REPORTING}; # Skip generating counters for this node
   ...
   my $erbs_file = "$output_dir/A${file_date_time}_$config->{dnPrefix}_statsfile.xml";
   mkpath($output_dir);

   open $config->{FILE_HANDLE}, '>', "$erbs_file" or croak "Cannot open file $erbs_file, $!";      
   print { $config->{FILE_HANDLE} } file_header( $config );

   for my $mo_type (sort keys %moids_for) {
      debug("      mo_type :  $mo_type");
      debug(" node_version :  $config->{nodeVersion}");

      $config->{measInfoId} = "PM=1,PmGroup=$mo_type";
      $config->{COUNTERS}   = $counter_formulas_for{ $config->{nodeVersion} }{$mo_type}{COUNTERS};
      $config->{FORMULAS}   = $counter_formulas_for{ $config->{nodeVersion} }{$mo_type}{FORMULAS};
   ...
   close $config->{FILE_HANDLE} or croak "Cannot close file $erbs_file, $!";   
}

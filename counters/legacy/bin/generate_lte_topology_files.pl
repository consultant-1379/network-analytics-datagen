...
my $node_counter_dir  = "/eniq/home/dcuser/counters/nodes/erbs/Counters";
my @node_config_files = get_configuration_info( root_dir => $root_dir, node_type => 'erbs' );
...
for my $node_config_file (@node_config_files) {
   ...
   my $fdn           = "SubNetwork=$config->{ROOT_MO},MeContext=$node";
   my $topology_dir  = "/eniq/data/pmdata/$config->{OSS_ID}/lte/topologyData/ERBS";
   my $topology_file = "$topology_dir/SubNetwork_$config->{ROOT_MO}_MeContext_${node}.xml";
   ...
   print {$TOPOLOGY_FH} erbs_topology( 
      ...
      fingerprint       => $node,
      MeContextId       => $node,
      siteRef           => "SubNetwork=$config->{ROOT_MO},Site=$config->{siteRef}",
...
sub erbs_topology {
  ...
  <mo fdn="SubNetwork=$args{CONFIG}->{ROOT_MO},MeContext=$args{eNBId},ManagedElement=1,ENodeBFunction=1" mimName="ERBS_NODE_MODEL">
...
sub EUtranCell_topology {
  ...
  <mo fdn="$args{fdn}" mimName="ERBS_NODE_MODEL">
    ...
    <attr name="sectorCarrierRef">
      <seq count="1">
        <item>SubNetwork=$args{CONFIG}->{ROOT_MO},MeContext=$args{MeContextId},ManagedElement=1,ENodeBFunction=1,SectorCarrier=$cell_id</item>
      </seq>
    </attr>
    ...
    <attr name="timestampOfChange">$args{timestampOfChange}</attr>
    <attr name="hostingDigitalUnit">SubNetwork=$args{CONFIG}->{ROOT_MO},MeContext=$args{MeContextId},ManagedElement=1,Equipment=1,Subrack=1,Slot=1,PlugInUnit=1</attr>
...
sub SectorCarrier_topology {
  ...
  <mo fdn="$args{fdn}" mimName="ERBS_NODE_MODEL">
    ...
    <attr name="reservedBy">
      <seq count="1">
        <item>SubNetwork=$args{CONFIG}->{ROOT_MO},MeContext=$args{MeContextId},ManagedElement=1,ENodeBFunction=1,$cell_type=$carrier_id</item>
      </seq>
    </attr>
    <attr name="timestampOfChange">$args{timestampOfChange}</attr>
